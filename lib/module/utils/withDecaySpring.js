...
export function withDecaySpring(userConfig, callback) {
  'worklet';
    ...
    function decaySpring(animation, now) {
      ...
      if (Array.isArray(config.clamp)) {
        if (animation.moveBack) {
          ...
          const underDampedVelocity = zeta * omega0 * underDampedFrag1 - underDampedEnvelope * (cos1 * (springV0 + zeta * omega0 * x0) - omega1 * x0 * sin1);
          ...
          const isDisplacement = config.stiffness === 0 || Math.abs(toValue - current) < REST_DISPLACEMENT_THRESHOLD;
          if (zeta < 1) {
